...
// #define YOLOV8_RTSP_PROCESS_SKIP_POSTPROCESS 0
...
static int RunOneStreamProcess(const StreamConfig &cfg, size_t streamIndex)
{
    ACLLITE_LOG_INFO("[process stream %zu] child pid=%d, model=%s, stream=%s",
                     streamIndex, static_cast<int>(getpid()), cfg.modelPath, cfg.url.c_str());

    SampleYOLOV8 sampleYOLO(cfg.modelPath, cfg.modelWidth, cfg.modelHeight, false,
                            cfg.maxLogDetPerFrame, cfg.deviceId);
    Result ret = sampleYOLO.InitResource();
    if (ret == FAILED)
    {
        ACLLITE_LOG_ERROR("[process stream %zu] InitResource failed, errorCode is %d", streamIndex, ret);
        return FAILED;
    }

    ::VideoCapture videoCapture(cfg.url, cfg.deviceId);
    AclLiteError aclRet = videoCapture.Open();
    if (aclRet != ACLLITE_OK)
    {
        ACLLITE_LOG_ERROR("[process stream %zu] open stream failed, errorCode is %d, url=%s",
    ...
    if (aclRet != ACLLITE_OK)
    {
        ACLLITE_LOG_ERROR("[process stream %zu] set rtsp transport failed, errorCode is %d", streamIndex, aclRet);
        videoCapture.Close();
        return FAILED;
    }
    ...
    while (cfg.frameLimit <= 0 || static_cast<int32_t>(processedCount) < cfg.frameLimit)
    {
        ...
        if (aclRet != ACLLITE_OK)
        {
            ACLLITE_LOG_ERROR("[process stream %zu] read decoded frame failed or stream ended, errorCode is %d",
                              streamIndex, aclRet);
            break;
        }
        ...
        if (ret == FAILED)
        {
            ACLLITE_LOG_ERROR("[process stream %zu] ProcessDecodedFrame failed, errorCode is %d", streamIndex, ret);
            videoCapture.Close();
            return FAILED;
        }
        ...
        if (ret == FAILED)
        {
            ACLLITE_LOG_ERROR("[process stream %zu] Inference failed, errorCode is %d", streamIndex, ret);
            videoCapture.Close();
            return FAILED;
        }
        ...
        if (ret == FAILED)
        {
            ACLLITE_LOG_ERROR("[process stream %zu] GetResult failed, errorCode is %d", streamIndex, ret);
            videoCapture.Close();
            return FAILED;
        }
        ...
        if (cfg.statsIntervalSec > 0)
        {
            ...
            if (elapsedSec >= static_cast<double>(cfg.statsIntervalSec) && intervalCount > 0)
            {
                ...
                ACLLITE_LOG_INFO("[process stream %zu] periodic average in last %.2fs (%zu frames): pull_decode=%.3fms, preprocess=%.3fms, inference=%.3fms, d2h=%.3fms, postprocess=%.3fms, total=%.3fms, fps=%.3f",
    ...
    if (processedCount == 0)
    {
        ACLLITE_LOG_ERROR("[process stream %zu] no frame processed from stream: %s", streamIndex, cfg.url.c_str());
        return FAILED;
    }
    ...
    ACLLITE_LOG_INFO("[process stream %zu] average timing(ms) for %zu frames: pull_decode=%.3f, preprocess=%.3f, inference=%.3f, d2h=%.3f, postprocess=%.3f, total=%.3f, fps=%.3f, url=%s",
...
int main(int argc, char *argv[])
{
    ...
    ACLLITE_LOG_INFO("process mode: streams=%zu, model=%s, input size=%d x %d, frameLimit=%d, maxLogDet=%zu, device=%d, transport=%s, statsIntervalSec=%d",
    ...
    for (size_t i = 0; i < streams.size(); ++i)
    {
        ...
        if (pid < 0)
        {
            ACLLITE_LOG_ERROR("fork failed for stream %zu", i);
            return FAILED;
        }
        if (pid == 0)
        {
            int childRet = RunOneStreamProcess(cfg, i);
            return childRet == SUCCESS ? 0 : 1;
        }
    ...
    {
        int status = 0;
        pid_t waitRet = waitpid(children[i], &status, 0);
        if (waitRet < 0)
        {
            ACLLITE_LOG_ERROR("waitpid failed for child pid=%d", static_cast<int>(children[i]));
            ++failedCount;
            continue;
        }
        if (!WIFEXITED(status) || WEXITSTATUS(status) != 0)
        {
            ACLLITE_LOG_ERROR("child pid=%d failed, status=%d", static_cast<int>(children[i]), status);
            ++failedCount;
        }
    }
    ...
    if (failedCount > 0)
    {
        ACLLITE_LOG_ERROR("process mode finished with %d failed streams out of %zu", failedCount, streams.size());
        return FAILED;
    }

    ACLLITE_LOG_INFO("process mode finished successfully, streams=%zu", streams.size());
    return SUCCESS;
}
